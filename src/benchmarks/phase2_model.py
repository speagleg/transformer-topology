"""Phase 2 model: Topological Reasoning with Hodge decomposition features."""
...
class TopologicalReasoningModel(nn.Module):
    """Model for Phase 2 topological reasoning benchmarks.

    Extends the Phase 1 model with:
    - Higher-order GNN message passing (via use_higher_order flag)
    - Hodge decomposition features in the classifier
    ...
    def __init__(self, embedding_dim: int, gnn_hidden: int, gnn_spatial_layers: int,
                 ...
                 max_classes: int, max_iterations: int = 5,
                 convergence_threshold: float = 0.1,
                 use_higher_order: bool = True,
                 use_topological_pe: bool = True):
        super().__init__()
        ...
        # Classifier input: query_emb + target_emb + diff_emb + hodge features (3 means)
        classifier_input_dim = 3 * embedding_dim + 3  # 3 hodge component means
