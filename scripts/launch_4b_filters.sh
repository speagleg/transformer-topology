#!/bin/bash
# Phase 4b — all spectral filter comparison
# Runs bandpass, wave_cosine, chebyshev, wavelet filters
# (heat and schrodinger already running in launch_4b_validate.sh)
#
# Reuses datasets already generated by the main run.
# 4 filters × 5 tasks × 1 variant (hierarchical) = 20 jobs
# Expected runtime: ~1-1.5 hours on 8x A100
#
# Usage:
#   ./scripts/launch_4b_filters.sh

set -e

DATASETS_DIR="data/benchmark_4b_datasets"
RESULTS_BASE="data/benchmark_4b_results"
LOG_DIR="$RESULTS_BASE/logs_filters"
JOBS_FILE="$RESULTS_BASE/jobs_filters.txt"

FILTERS="bandpass wave_cosine chebyshev wavelet"
TASKS="diverse propagation_delay spectral_gap hodge_class bfs"

echo "============================================================"
echo "Phase 4b — Spectral Filter Comparison"
echo "============================================================"
...
if [ "$EXPECTED" -lt 25 ]; then
    ...
    echo "Run launch_4b_validate.sh first to generate datasets."
    exit 1
fi
...
for FILTER in $FILTERS; do
    CONFIG="config/benchmark_4b_${FILTER}.yaml"
    RESULTS_DIR="$RESULTS_BASE/jobs_${FILTER}"
    mkdir -p "$RESULTS_DIR"
...
# Merge results per filter
echo ""
echo "=== Merging results ==="
for FILTER in $FILTERS; do
    RESULTS_DIR="$RESULTS_BASE/jobs_${FILTER}"
    if ls "$RESULTS_DIR"/result_*.json 1>/dev/null 2>&1; then
        python3 scripts/merge_results.py "$RESULTS_DIR" --output "$RESULTS_BASE/results_${FILTER}.json"
    fi
done

echo ""
echo "=== Done! ==="
echo "Results per filter:"
for FILTER in $FILTERS; do
    echo "  $FILTER: $RESULTS_BASE/results_${FILTER}.json"
done
echo "Logs: $LOG_DIR/"
